...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, no_fine, render_test=False, reconstruct=False):
    ...
    gtdepth_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "gt_depth")
    masks_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "masks")
    ...
    if len(mask_list) != 0:
        for image in tqdm(mask_list):
            image = image.float()
            torchvision.utils.save_image(image, os.path.join(
                masks_path, '{0:05d}'.format(count) + ".png"))
            count += 1
    ...
    if len(render_depths) != 0:
        for image in tqdm(render_depths):
            image = np.clip(image.cpu().squeeze(
            ).numpy().astype(np.uint8), 0, 255)
            cv2.imwrite(os.path.join(
                depth_path, '{0:05d}'.format(count) + ".png"), image)
            count += 1
...
def render_sets(dataset: ModelParams, hyperparam, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool, skip_video: bool, reconstruct: bool):
    ...
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration,
                      shuffle=False, load_coarse=dataset.no_fine)
        ...
        if not skip_train:
            ...
            ), gaussians, pipeline, background, dataset.no_fine, reconstruct=reconstruct)
        if not skip_test:
        ...
        if not skip_video:
            ...
            ), gaussians, pipeline, background, dataset.no_fine, render_test=True, reconstruct=reconstruct)
...
def reconstruct_point_cloud(images, masks, depths, camera_parameters, name):
    ...
    for i_frame in frames:
        ...
        rgb_np = rgb_tensor.mul(255).add_(0.5).clamp_(
        ...
        pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            ...
            o3d.camera.PinholeCameraIntrinsic(
                width, height, focal_x, focal_y, width / 2, width / 2),
            project_valid_depth_only=True
        )
        o3d.io.write_point_cloud(os.path.join(
            output_frame_folder, 'frame_{}.ply'.format(i_frame)), pcd)
...
if __name__ == "__main__":
    ...
    if args.configs:
        ...
        from mengine.config import Config
        config = Config.fromfile(args.configs)
        args = merge_hparams(args, config)
    # Initialize system state (RNG)
    safe_state(args.quiet)
    render_sets(model.extract(args), hyperparam.extract(args), args.iteration, pipeline.extract(
        args), args.skip_train, args.skip_test, args.skip_video, args.reconstruct)
